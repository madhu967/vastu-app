...
// 1. Update generateVastuPdf.ts
let pdfContent = fs.readFileSync('src/pdf/generateVastuPdf.ts', 'utf8');

const buildRowsRegex = /const buildRows = \(table: ResultTable\) => \{[\s\S]+?\}\)\.join\(""\);\n\};\n/m;
  ...
  const getPhalaData = (key: string, roundedStr: string): [string, string, string] => {
    ...
    switch (key) {
      ...
      case "Dikruti": {
        ...
        const teDik = ["ఇంద్రుడు", "అग्ని", "యముడు", "నిరృతి", "వరుణుడు", "వాయువు", "కుబేరుడు", "ఈశానుడు"];
...
if (buildRowsRegex.test(pdfContent)) {
  pdfContent = pdfContent.replace(buildRowsRegex, newBuildRows);
  // Inject form into buildRows call
  pdfContent = pdfContent.replace(/const dataRows = buildRows\(table\);/, 'const dataRows = buildRows(table, form);');
} else {
  console.error("Could not find buildRows in pdfContent");
}

// Ensure titles translate to Viswakarma Vastu Sarvaswam
...
if (!pdfContent.includes('const getPdfTranslations')) {
  // If getPdfTranslations isn't there (meaning we are on the unpatched file)
  ...
  // Remove the hardcoded old phala column header logic if there was one, or just replace it
  pdfContent = pdfContent.replace(/<div style="width:100%; border-bottom:1px solid rgba\(212,175,55,0\.35\); text-align:center; padding:3px; box-sizing:border-box;">ఫల విశ్లేషణ<\/div>/,
    '<div style="width:100%; border-bottom:1px solid rgba(212,175,55,0.35); text-align:center; padding:3px; box-sizing:border-box;">${T.phalaHeader}</div>');
  pdfContent = pdfContent.replace(/<div style="flex:1; border-right:1px solid rgba\(212,175,55,0\.35\); text-align:center; padding:3px; font-size:11px; box-sizing:border-box;">వివరాలు<\/div>/,
    '<div style="flex:1; border-right:1px solid rgba(212,175,55,0.35); text-align:center; padding:3px; font-size:11px; box-sizing:border-box;">${T.phalaDetails}</div>');
  pdfContent = pdfContent.replace(/<div style="flex:1; text-align:center; padding:3px; font-size:11px; box-sizing:border-box;">ఫలితం<\/div>/,
    '<div style="flex:1; text-align:center; padding:3px; font-size:11px; box-sizing:border-box;">${T.phalaResult}</div>');
...
console.log('Update script completely written and saved!');
